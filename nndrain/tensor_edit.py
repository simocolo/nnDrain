...
class TensorEdit:
    ...
    def weights_drain(self, p_drain, threshold_coeff=2.0, condition=True):
        ...
        :param threshold_coeff: threshold coefficient. Multiplied by the tensor threshold allows us to obtain the actual range where the decay is applied [-r_value; r_value]
        :param condition: if condition to execute the method
        :return: None
        """
    ...
    def set_threshold(self, layer):
        """
        sets the threshold value of the layer. the latter is used by the class to calibrate the drain and remove contribution

        :param layer: the layer to set the threshold value
        :return: None 
        """
    ...
    def weights_remove(self, p_remove=0.1, threshold_coeff=0.8, max_removal=1.0, condition=True, verbose=False):
        ...
        :param threshold_coeff: threshold coefficient. Multiplied by the tensor threshold allows us to obtain the actual threshold for which if all the values of a row or column are below it, the row or column is removed
